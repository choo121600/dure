...
export const TIMING: TimingConfig = {
  ...
  DEBOUNCE_MS: parseEnvInt(process.env.DURE_DEBOUNCE_MS, 2000),

  /** Delay after starting Claude before sending commands */
  CLAUDE_STARTUP_DELAY_MS: parseEnvInt(process.env.DURE_CLAUDE_STARTUP_DELAY_MS, 2000),

  /** Delay after paste completion */
  PASTE_COMPLETION_DELAY_MS: parseEnvInt(process.env.DURE_PASTE_COMPLETION_DELAY_MS, 500),

  /** Delay for CRP detection */
  CRP_DETECTION_DELAY_MS: parseEnvInt(process.env.DURE_CRP_DETECTION_DELAY_MS, 1000),

  /** Interval for checking agent activity (default 30 seconds) */
  ACTIVITY_CHECK_INTERVAL_MS: parseEnvInt(process.env.DURE_ACTIVITY_CHECK_INTERVAL_MS, 30000),

  /** Maximum inactivity time before considering agent stuck (default 2 minutes) */
  MAX_INACTIVITY_TIME_MS: parseEnvInt(process.env.DURE_MAX_INACTIVITY_TIME_MS, 120000),

  /** File watcher stability threshold */
  FILE_WATCHER_STABILITY_MS: parseEnvInt(process.env.DURE_FILE_WATCHER_STABILITY_MS, 500),

  /** File watcher poll interval */
  FILE_WATCHER_POLL_INTERVAL_MS: parseEnvInt(process.env.DURE_FILE_WATCHER_POLL_INTERVAL_MS, 100),

  /** Default wait timeout for file operations (5 minutes) */
  DEFAULT_FILE_WAIT_TIMEOUT_MS: parseEnvInt(process.env.DURE_DEFAULT_FILE_WAIT_TIMEOUT_MS, 300000),

  /** Graceful shutdown timeout */
  GRACEFUL_SHUTDOWN_TIMEOUT_MS: parseEnvInt(process.env.DURE_GRACEFUL_SHUTDOWN_TIMEOUT_MS, 5000),

  /** Output streamer default polling interval */
  OUTPUT_POLLING_INTERVAL_MS: parseEnvInt(process.env.DURE_OUTPUT_POLLING_INTERVAL_MS, 1000),

  /** Minimum output polling interval (fast) */
  OUTPUT_POLLING_MIN_MS: parseEnvInt(process.env.DURE_OUTPUT_POLLING_MIN_MS, 100),

  /** Maximum output polling interval (slow) */
  OUTPUT_POLLING_MAX_MS: parseEnvInt(process.env.DURE_OUTPUT_POLLING_MAX_MS, 2000),

  /** Recovery strategy delay between operations */
  RECOVERY_DELAY_MS: parseEnvInt(process.env.DURE_RECOVERY_DELAY_MS, 1000),

  /** Default timeout extension (5 minutes) */
  DEFAULT_TIMEOUT_EXTENSION_MS: parseEnvInt(process.env.DURE_DEFAULT_TIMEOUT_EXTENSION_MS, 300000),

  /** Maximum timeout extension (30 minutes) */
  MAX_TIMEOUT_EXTENSION_MS: parseEnvInt(process.env.DURE_MAX_TIMEOUT_EXTENSION_MS, 1800000),

  /** Delay before browser opens after server start */
  BROWSER_OPEN_DELAY_MS: parseEnvInt(process.env.DURE_BROWSER_OPEN_DELAY_MS, 1500),

  /** Delay before stopping tmux session */
  TMUX_STOP_DELAY_MS: parseEnvInt(process.env.DURE_TMUX_STOP_DELAY_MS, 500),

  /** Usage tracker refresh interval (10 seconds) */
  USAGE_TRACKER_REFRESH_MS: parseEnvInt(process.env.DURE_USAGE_TRACKER_REFRESH_MS, 10000),

  /** Inactive pane check threshold */
  INACTIVE_PANE_THRESHOLD_MS: parseEnvInt(process.env.DURE_INACTIVE_PANE_THRESHOLD_MS, 5000),

  /** Retry base delay */
  RETRY_BASE_DELAY_MS: parseEnvInt(process.env.DURE_RETRY_BASE_DELAY_MS, 1000),

  /** Retry max delay */
  RETRY_MAX_DELAY_MS: parseEnvInt(process.env.DURE_RETRY_MAX_DELAY_MS, 30000),
} as const;
...
export const AGENT_TIMEOUTS: AgentTimeoutsConfig = {
  ...
  REFINER_MS: parseEnvInt(process.env.DURE_REFINER_TIMEOUT_MS, 300000),

  /** Builder timeout (10 minutes) */
  BUILDER_MS: parseEnvInt(process.env.DURE_BUILDER_TIMEOUT_MS, 600000),

  /** Verifier timeout (5 minutes) */
  VERIFIER_MS: parseEnvInt(process.env.DURE_VERIFIER_TIMEOUT_MS, 300000),

  /** Gatekeeper timeout (5 minutes) */
  GATEKEEPER_MS: parseEnvInt(process.env.DURE_GATEKEEPER_TIMEOUT_MS, 300000),
} as const;
...
export const CACHE: CacheConfig = {
  ...
  STATE_CACHE_TTL_MS: parseEnvInt(process.env.DURE_STATE_CACHE_TTL_MS, 1000),
} as const;
