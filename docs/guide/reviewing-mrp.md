...
# Verifier 로그
cat .orchestral/runs/{run_id}/verifier/log.md
...
Generated by Orchestral run-{timestamp}
...
# 웹 UI에서 "Download" 클릭
# 또는
tar -czf mrp.tar.gz .orchestral/runs/{run_id}/mrp/
```
...
```bash
# 코드 확인
cat .orchestral/runs/{run_id}/mrp/code/src/utils/formatDate.ts
...
```bash
# 코드 확인 (복잡함)
cat .orchestral/runs/{run_id}/mrp/code/src/api/users.ts

# 로그 확인 (왜 2번 재시도?)
cat .orchestral/runs/{run_id}/gatekeeper/review.md

# 실패한 테스트 확인
cat .orchestral/runs/{run_id}/verifier/results.json
```
...
diff -u src/services/UserService.ts \
  .orchestral/runs/{run_id}/mrp/code/src/services/UserService.ts
...
MRP_DIR=".orchestral/runs/$RUN_ID/mrp"
